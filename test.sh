...
readonly VERSION_MONO="4.8.0"
readonly VERSION_NODE="v6.10.1"
readonly VERSION_PERL="v5.18.2"
readonly VERSION_PHP="5.6.30-7"
readonly VERSION_PYTHON2="2.7.11"
readonly VERSION_PYTHON3="3.6.1"
readonly VERSION_RUBY="2.3"
...
docker_run () {
	...
	docker run --rm -it givery/codecheck $*
}
docker_run_login () {
	echo "===== command to run: bash --login -c \"$*\" =====" >&2
	docker run --rm -it givery/codecheck bash --login -c "$*"
}
